...
app.post('/usuarios', (req, res) => {
    ...
    pool.connect((err, client) => {
        ...
        //selecione da tabela usuario onde o email é igual o que está sendo enviado no body
        //se retornar linha > 0 é porque ele achou no banco de dados um email igual, se não
        //ele cadastra
        var sql = 'select from usuario where email=$1'
        var dados1 = [req.body.email]
        client.query(sql, dados1, (err, result) => {
            if(result.rowCount>0) {
                return res.status(500).send({message:'usuario ja cadastrado'})
            } else {
